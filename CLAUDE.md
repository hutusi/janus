...
Single binary (`cmd/janus`); all logic under `internal/`, depending inward on `internal/model`. Run lifecycle: `server` → `runner.Trigger` → `workspace.Checkout` → `pipeline.Parse` → match `on:` → `engine.NewRun`/`SaveRun` → async `engine.Execute` (DAG scheduler → per-step `os/exec`).
