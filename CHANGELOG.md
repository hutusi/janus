...
- **Per-trigger pipeline file** — `POST /api/trigger` accepts an optional `pipeline_path` field selecting a committed pipeline file other than the configured default (e.g. `.janus/release.yml`) for that one run. The path is confined to the checkout — absolute paths and `..` escapes are rejected with `400` before anything is cloned — and the override is recorded on the run's event, so `GET /api/runs/{id}` shows which file ran. Webhooks keep using the server-wide `pipeline_path`.
