...
| `GET /api/runs/{id}/logs`         | Combined logs; `?job=&step=` for one step; `?follow=1` to stream (bounded: 32 concurrent followers, 15 min per stream — a saturated server answers `503` + `Retry-After`, and a stream that hits the limit ends with a marker; reconnect to continue) |
...
| `GET /` and `/runs/{id}`          | Read-only HTML dashboard: statuses plus run/job/step durations (in-progress ones tick live); paginated run list (`?page=N`); auto-refreshes while runs are active (`/favicon.svg` serves the logo) |
