...
| `history_limit` | `--history-limit` | `0` | Max terminal runs to retain. When exceeded, the oldest terminal runs — and their logs — are deleted after each run finishes and at startup; running/pending runs are never pruned. `0` means unlimited; negatives are a startup error. The flat-file store lists runs by reading every run directory, so a limit here is also what keeps listing and startup fast. |
