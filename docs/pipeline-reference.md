...
- Absurd sizes — more than 256 jobs, more than 256 steps in a job, a job name
  over 256 characters, or a step `run` over 64 KiB. Generous limits that only
  reject the pathological, so per-run artifacts stay finite.
